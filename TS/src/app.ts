...
let prova1: number;// se non so ancora il numero
let prova2: number = 5;// ridondante, lo capisce da solo
...
function inference(num1: any, num2 = 0) { };
// inference("3", "3");
...
// OBJECT
let persona2: {} = {// tipo object esplicitamente
    nome: "Mario",
    cognome: "Rossi"
}

let persona3 = {// tipo object lo capisce da solo
    nome: "Mario",
    cognome: "Rossi"
}
...
let persona4: {
    ...
    cognome: string;
    eta: number;
    indirizzo: {// nested object
        via: string;
        civico: number;
        citta: string;
    }
}
persona4 = {// avvisa del fatto che manca una prop se non la scrivo
    nome: "Paolo",
    cognome: "Azzurri",
    eta: 30,// con stringa darebbe errore
    indirizzo: {
        via: "G. Leopardi",
        civico: 5,
        citta: "Gassino To"
...
// ARRAY
const arr: number[] = [1, 2, 3];// mi dice const arr: number[]
// const arr1: string[] = [1, 2, 3];// mi avvisa Type 'number' is not assignable to type 'string'
const arr1: string[] = ['', 'ciao', "", ``];// non da problemi
const arr2: any[] = ["ciccio", 2, true, [], {}];//any può contentere QUALSIASI TIPO
...
const tupla: [number, number] = [1, 2];// specifico cosa accettto nell'array, SOLO 2 e SOLO di quel tipo
const tupla1: [number, string, boolean] = [1, "ciccia", true];



// ANY, UNION, CUSTOM TYPE, ENUM
// ANY, il più generico possibile
let myany: [any, number] = ["ciccino", 4];// es tupla con primo valore any
let myany1: [any, number] = [{}, 4];
let myany2: {
    nome: any;// non ha tanto senso, so che è una stringa il nome
}
...
let myunion: string | number = "ciccio";
let myunion1: string | number = 2;
let myunion3: string | number | string[] = ["polpettone", "polpettina"];
let myunion4: string | object | boolean = { nome: "cinzia", cognome: "vittone", indirizzo: { via: "Leopardi", civico: 5 } };// obj
let myunion5: string | number | any[] = [2, "porcelli"];
...
function trovaUtente(utente: { nome: string; cognome: string }) { };
// ecco perchè si usano i tipi customizzati
type Utente1 = {
    nome: string;
    cognome: string;
    eta: number;
...
function trovaUtente1(utente1: Utente1) { };// qua si prende in automatico il tipo
// trovaUtente1({});// errore, vuole per forza nome, cognome, eta per rispettare il custom dichiarato

type Utente2 = object | string;// il custom type accetterà questi tipi
let utente2: Utente2 = "ciccio";
let utente3: Utente2 = {};
...
enum RuoloEsempio {// posso anche dare io una posizione predefinita
    ADMIN = 100,// 0 default, poi da dove scrivo riparte a contare i prossimi 
    // (enum member) RuoloEsempio.ADMIN = 0 dice la posizione
    USER = 1000,// 1 default
    GUEST// 2 default, 1001 perchè riparte da 1000
}
...
if (dipendente4.ruolo === RuoloEsempio1.ADMIN) { };// non sono uguali
if (dipendente4.ruolo === RuoloEsempio1.USER) { };// sono uguali
...
function somma1(num1: any, num2: any) {// si lamenta se lascio (num1, num2)
...
*/
function somma3(num1: number, num2 = 10) {// default, se non passo niente, inoltre capisce (parameter) num2: number
    console.log(num1 + num2);
}
somma3(3, 3);// 6
somma3(3);// 13

function somma4(num1: number, num2: number = 20) {// default, se non passo niente, inoltre capisce (parameter) num2: number
    console.log(num1 + num2);
}

// 3. IL TIPO DI RITORNO
function somma5(num1: number, num2 = 1) {
    return num1 + num2;
}
...
function somma6(num1: number, num2 = 1) {// num2? opzionale
    return "stringa";
}
...
function somma7(num1: number, num2 = 1): number {
...
function somma8(num1: number, num2 = 1): number {
    // console.log();// A function whose declared type is neither 'void' nor 'any' must return a value
    return num1 + num2;
}

// utilizzo ANY
function somma9(num1: number, num2 = 1): any {// accetta davvero tutto
    if (num1 === num2) {
        return num1 + num2;
    } else {
        console.log();
...
// utilizzo VOID
function somma10(num1: number, num2 = 1): void {
...
// utilizzo UNDEFINED
function somma11(num1: number, num2 = 1): undefined {
    return undefined;// unico tipo che accetta
}

// 4. FUNZIONI COME TIPO DI VARIABILE
function funzioneComeVariabile(num1: number, num2 = 1): number {
    return num1 + num2;
}

function funzioneComeVariabile1(str: string): void {
    console.log(str);
}
...
let provaY: (x: number, y: number) => number;
...
function provaW(num: number, cb: () => void) { }
provaW(6, () => {
...
// opzione 3
let personaC: Persona = new Persona();// variabile: tipo = assegnazione
let personaC1: Persona = new Persona();
let personaC2: Persona = new Persona();
...
class MyPersona {// 11.
    ...
    constructor(private nome: string, cognome: string, protected readonly genere: string, protected id: string) {// tipizzati
    ...
    static parla(): void {// 10.
        console.log("Fa davvero caldo oggi!");
    }
}
let myPersona1: MyPersona = new MyPersona("Ciccio", "Pallino", "M", "01");// devo per forza mettere gli argomenti ora, devono essere di QUEL tipo
...
let myStudente1: MyStudente = new MyStudente("Ciccino", "Piccino", "M", "03");// Expected 3 arguments, but got 0. eredita da MyPersona
...
let myInsegnante1: MyInsegnante = new MyInsegnante("Luca", "Rossi", "M", "04", "Italiano", ["Pippo", "Pluto", "Paperino"]);
...
abstract class Esempio {
    ...
    abstract scrivi(): void;// 11. non va implementato qui ma in TUTTE le classi figlie
...
class MyPreside {
    private static instance: MyPreside;// UNICA ISTANZA
...
// INTERFACCE
// usate per essere implementate nelle classi in modo trasversale
// INTERFACCIA ha tutti metodi astratti di default, usate per essere implementati nelle classi
interface ProvaInterfaccia {
    // no modificatori
    readonly nome: string
    // non implementato ma solo il tipo di ritorno
    saluta(): void;
}

interface ProvaInterfaccia2 {
    scrivi(): void;
}

// CLASSE ASTRATTA può avere sia metodi normali che astratti
abstract class ProvaAstratta {
    saluta() {
    }
    abstract prova(): void;
}

// CUSTOM TYPE usati per creare tipi più complessi che non esistono, es union
type Custom = {
    nome: string;
    saluta(): void;
}
...
class X implements ProvaInterfaccia, ProvaInterfaccia2 {
    nome: string;
    // no modificatori
...
// estende Y(che a sua volta estende J) e Z, SOLO con le interfacce si può far derivare da più di 1, con le classi NO 
interface W extends Y, Z {
    via: string;
    mangia(): void;
}
class Abitante implements W {
